...
def test_calcBestPrice():
    ...
    assert cb.calcBestPrice(advert_info=a, current_place=2, current_price=200, target_place=4) == cb.Decision(True, 5)
    assert cb.calcBestPrice(advert_info=a, current_place=3, current_price=100, target_place=3) == cb.Decision(True, 5)
    ...
    a = cb.AdvertInfo()
    a.placies.append(cb.AdvertPlace(position=1, price=300))
    assert cb.calcBestPrice(advert_info=a, current_place=2, current_price=200, target_place=2) == cb.Decision(True, 5)
    # assert cb.calcBestPrice(advert_info=a, current_price=200, target_place=2) == cb.Decision(True, 5)

    
def test_AdvertInfo():
    json_data = [{'code': '', 'advertId': 2487286, 'id': 93089486, 'cpm': 865, 'subject': 4683}, 
        {'code': '', 'advertId': 3964213, 'id': 97662406, 'cpm': 864, 'subject': 4683}, 
        {'code': '', 'advertId': 3794903, 'id': 17762592, 'cpm': 858, 'subject': 4683}]
    ai = cb.AdvertInfo()
    ai.fromAdverts(json_data)
    for i in range(len(ai.placies)):
        assert ai.placies[i].position == i
    assert ai.placies[0].price == 865
    assert ai.placies[1].price == 864
    assert ai.placies[2].price == 858